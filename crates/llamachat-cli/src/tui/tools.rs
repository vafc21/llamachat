...
// Re-export shared types from the core crate so the CLI and desktop use identical
// PermMode / Effort enums with the same methods everywhere.
pub use llamachat_core::{Effort, PermMode};
...
// ── (PermMode + Effort now live in llamachat_core::types — re-exported above) ──
